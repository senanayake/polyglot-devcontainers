#!/usr/bin/env python3
"""Evaluate pip-audit findings against the local security policy."""
...
def resolve_osv_metadata(
    ...
    url = f"https://api.osv.dev/v1/vulns/{parse.quote(advisory_id, safe='')}"
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"expected advisory payload object for {advisory_id}")
...
def evaluate_findings(audit_payload: dict[str, Any], policy: dict[str, Any]) -> dict[str, Any]:
    ...
    for dependency in audit_payload.get("dependencies", []):
        for vulnerability in dependency.get("vulns", []):
            ...
            fix_versions = [str(value) for value in vulnerability.get("fix_versions", [])]
            ...
            "accepted_advisories": [
                {
                    ...
                    "expires_on": accepted.expires_on.isoformat() if accepted.expires_on else None,
                }
...
def markdown_report(summary: dict[str, Any], report_path: Path, policy_path: Path) -> str:
    lines = [
    ...
    if not summary["findings"]:
        lines.append(
            "| `none` | `-` | `-` | `-` | `-` | `continued` | "
            "No Python package advisories were reported. |"
        )
